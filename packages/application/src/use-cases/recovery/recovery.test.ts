...
describe("recovery", () => {
  it("does not start a new run for an unknown process", async () => {
    ...
    app.markRunUnknown(started.run.id);
    const result = await app.reconcile(created.project.id);
    expect(result.unknown).toEqual([started.run.id]);
    expect(app.world.runs.size).toBe(1);
    expect(app.world.activeRunForTask(task.id)?.id).toBe(started.run.id);
  });
});
