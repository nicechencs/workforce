...
**日期：** 2026-09-11  
**修订：** 2026-09-12 — §9：V0.1 会话传输冻结为 Desktop-local / in-process（无 Daemon chat path）；T02 已冻结会话 / 草稿 DTO；T20 发送仍禁用；编排 Agent 仍未实现。同日 §10：T21 项目启动面选择控件已探针/部分接线；无 probe 时 direct disabled；Task 详情不挂未接线控件。不宣称 M8 完成。2026-09-11 — §9：Desktop 作者面壳 + 写 API 落草稿已有；会话协议 / 编排 Agent 仍未实现。同日 §2/§3 对齐 D19：默认本机；远程与容器是一等 Placement；V0.1 隔离仍是 worktree，容器 runner 未实现。同日主循环改为项目制：Project → 编排 Team → 编排 Tasks → 编排 Workflow。增补 §7 画布与 §8 自定义 Team（M7）。同日增补 §9 对话生成（D17 / M7 planned）与 §10 双执行模式（D18 / M8 planned）。2026-09-10 — §1 映射到 IA §4.3.4（Settings 绑定，页头命令）。

## 1. 创建并运行项目

产品主对象是 Project。围着该项目：编排 Team（M3 选预设；M7 可自定义或由对话生成角色草稿）、编排 Tasks、编排 Workflow（M3 只读目录；M7 对话生成草稿后进画布编辑），再执行与验收（M8：每个 Agent 可绑定已发布工作流或直接执行）。桌面 V0.1 把“绑定 Workspace”映射到项目详情 **Settings**（IA §4.3.4）。开始规划 / 确认计划 / 开始执行留在页头，不随标签卸载。D17 会话 / 草稿 DTO **已冻结**；V0.1 传输为 Desktop-local / in-process（无 Daemon chat）；对话发送 / 编排 Agent **尚未实现**（T20 仅有作者面壳）。D18 字段已冻结；T21 项目启动面已探针 + composed start 透传（#26 headed smoke PASS；#28 仅 unit/composed），M8 **未完成**。
...
## 9. 对话生成工作流（M7；作者面壳已有，会话 DTO 与 Desktop-local 传输已冻结，发送尚未接线）

用户与编排 Agent 对话，生成可编辑的工作流（及可选的角色/任务草稿），再进入 D15 画布。Desktop 已有「对话生成」作者面壳：T02 已冻结 `AuthoringSessionDto` / `AuthoringDraftDto`；V0.1 传输已冻结为 Desktop-local / in-process（无 Daemon chat 资源）。发送对话仍禁用直至后续 Desktop 会话存储 + 仅用户 append 接线；结构化名称可走已接通写接口落未发布草稿。这不是编排 Agent 已接通，也不是 M7 完成。
