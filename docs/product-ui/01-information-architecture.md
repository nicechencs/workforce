...
**版本：** V0.1 Draft  
**状态：** Product UI baseline（§1 项目制主对象；§4.3 为项目详情现行规范；§4.6 含角色版本库；§6.4 全局 Chat 壳；§6.2 / §4.6 含 D17/D18 planned）  
**日期：** 2026-09-13  
**修订：** 2026-09-13 — 我的角色版本库挂「AI 团队」；Chat 为壳按钮（§6.4），不是作者页专属、也不是 IM。Marketplace 现在不是一等面。均 **planned**。2026-09-11 — 产品主对象定为**项目制**（决策登记 §0）：围着一个 Project 编排 Team、Tasks、Workflow（含自定义 Team 与画布）。D15/D16 是该循环的必达环节，不是外挂。只读 `GET /workflows` 是已接通的 M3 过渡切片。同日补 D17 对话生成工作流、D18 按 Agent 双执行模式（均 **planned / 未实现**）。§4.3.2：已发布执行图的公开 Task DTO 必须返回 `dependsOn`；UI 展示真实依赖边，字段缺失时才写“依赖：未返回”。2026-09-10 — §2：`P0`/`P1` 是交付切片深度，不是侧栏可见性；一级导航全部出现在左侧栏。§4.3 从“建议标签”改为现行条款，并消解与线框 §3 的冲突。
...
主循环：`Project → Team → Tasks → Workflow 编排 → 执行与验收`（[决策登记 §0](../planning/decision-register.md#0-产品模型项目制)）。画布、自定义 Team、**角色版本库**、**全局 Chat 壳**与双执行模式是这条循环上的页面能力。Chat 不是与项目并列的业务对象。角色库、全局 Chat、D17/D18 **尚未实现**，页面不得假成功。
...
### 6.4 全局 Chat 壳（V0.1 就要有，planned）

Chat 是**壳按钮**（任意页可开，建议路由 `/chat`），不是一级侧栏「某员工私聊」，也不是只钉在工作流作者页。HTTP 未就绪时打开空壳并说明未接通，不得画可点击成功。

四类意图（分类结果不是完成）：

1. **创建角色** — 提案 → 用户确认 → 库中未发布草稿；可跳「我的角色版本库」
2. **创建流程** — 必须有 `projectId` → AuthoringSession → WorkflowDraft → 可去画布；未发布不执行
3. **问进度** — 只渲染已有 Task / Run / Event / Artifact；无记录说还没有；不出现无事实的「做完了」
4. **交流工作** — 无 `projectId` 先问项目；执行中再挂 `runId` 才走已有 Run 输入；没有挂点不发写

仍禁止：Worker IM、无项目聊天室、聊天当完成、把「去做」画成已在跑（D18 未就绪则诚实失败）。
