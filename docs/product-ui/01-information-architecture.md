...
**版本：** V0.1 Draft  
**状态：** Product UI baseline（§1 项目制主对象；§4.3 为项目详情现行规范；§4.6 含角色版本库；§6.4 全局 Chat；§6.2 / §4.6 含 D17/D18）  
**日期：** 2026-09-13  
**修订：** 2026-09-13 — 库页与 Chat 壳 **已接线**；卡片三字段、句意分类、空闲写卡 **planned**。建/改角色走库，不经 AuthoringSession。Marketplace 现在不是一等面。2026-09-11 — 产品主对象定为**项目制**（决策登记 §0）：围着一个 Project 编排 Team、Tasks、Workflow（含自定义 Team 与画布）。D15/D16 是该循环的必达环节，不是外挂。只读 `GET /workflows` 是已接通的 M3 过渡切片。同日补 D17 对话生成工作流、D18 按 Agent 双执行模式（均 **planned / 未实现**）。§4.3.2：已发布执行图的公开 Task DTO 必须返回 `dependsOn`；UI 展示真实依赖边，字段缺失时才写“依赖：未返回”。2026-09-10 — §2：`P0`/`P1` 是交付切片深度，不是侧栏可见性；一级导航全部出现在左侧栏。§4.3 从“建议标签”改为现行条款，并消解与线框 §3 的冲突。

## 1. 设计目标

**主对象是项目。** Workforce 客户端围着一个 Project 工作，而不是先做一个团队工作室或通用工作流 IDE。围绕当前项目，用户：

1. 编排 / 配置 **Team**（M3 只读预设；M7 从**我的角色版本库**选用/fork `WorkerVersion`）
2. 编排 **Tasks**
3. 编排 **Workflow**（必须高度可定制：M3 只读目录；M7 对话生成草稿 + 可视化画布/结构化编辑）
4. 执行、审批、验收产出（M8：每个 Agent 可绑定已发布工作流或直接执行）

主循环：`Project → Team → Tasks → Workflow 编排 → 执行与验收`（[决策登记 §0](../planning/decision-register.md#0-产品模型项目制)）。画布、自定义 Team、**角色版本库**、**全局 Chat**与双执行模式是这条循环上的页面能力。Chat 不是与项目并列的业务对象。库页与 Chat 壳已接线；卡片三字段、句意分类、空闲写卡、D18 direct **尚未实现**，不得假成功。
...
| AI 团队 | Team、WorkerVersion、角色版本库 | 为项目配团队：打开**我的角色版本库**选用/fork（M3 只读预设；M7 自定义）。服务项目循环，不是花名册或 IM | P0 |
...
- **P1**：仍是本表中的一级导航，**必须出现在左侧栏**。M3 深度可以更薄（只读目录已接通）；M7 要求画布编辑器、对话生成草稿与可写 Team（[D15](../planning/decision-register.md#d15-可视化工作流画布编辑器) / [D16](../planning/decision-register.md#d16-自定义-team-编排) / [D17](../planning/decision-register.md#d17-对话式-agent-编排工作流)）。M8 要求按 Agent 诚实选择绑定工作流或直接执行（[D18](../planning/decision-register.md#d18-双执行模式绑定工作流与直接执行)）。

不要把 P1 理解成隐藏入口。线框若只画了部分 P1 项，以本表为准，并回改线框。壳实现用 `primary: true` 表示侧栏可见，用 `priority: "p0" | "p1"` 表示切片深度。

产品**必须**能在项目循环里高度定制 Workflow：用对话生成草稿、用可视化画布/结构化面编辑，并用自定义 Team 给项目配团队（M7）。**现在就要有**我的角色版本库与全局 Chat 壳（壳按钮，任意页可开，不要做成「某员工私聊」）。每个 Agent 执行时必须能诚实选择绑定已发布工作流或直接执行（M8）。M3 可用只读模板/预设作为过渡切片，不得再写成「V0.1 不做画布 / 对话生成 / 自定义 Team / 双执行后置 / 不要 Chat / 库页后铺」。未实现前不得渲染可点击成功态。
...
3. Tasks 在本页 `tasks` 标签编排/查看；Workflow 定义在「工作流」编排后绑定到本项目（M3 只读目录；M7 对话生成草稿后进画布编辑）。M8 起，该项目内每个 Agent 启动前须能选择跟随已发布图或直接执行（无能力则禁用）。
...
目的：给**当前/后续项目**编排数字员工团队。入口在一级导航「AI 团队」。**找员工 = 打开我的角色版本库**（或用 Chat 说「找上次那个 reviewer」，仍查询同一座库）。不是独立花名册，也不是 IM。
...
**M7（必达；库页已接线，卡片三字段 planned）：**

- **我的角色版本库**可列出/搜已发布与草稿；看被哪些 TeamVersion 引用；归档后选择器不再提供该版本；fork 得到新草稿；未发布不可当已发布员工
- 详情必印卡片：他是谁 / 怎么干活 / 技能（空则显式空，不编造；不把 Runtime / Policy 画成必填）
- 创建自定义 Team 草稿：添加成员必须选已发布 WorkerVersion（或 fork 后再选）；选用时能看见该版本卡片；数量仍可编；发布请求带 `workerVersionId`
- 新项目可绑定已发布自定义 TeamVersion；未发布草稿或成员缺已发布 `workerVersionId` 不能开始规划
- 编辑已发布编排必须新建 version；不得改仍被引用的已发布 WorkerVersion
- Chat 建/改角色草稿确认后进同一座库，再在 Team 编辑面选用；空闲说话写入卡片，不经 AuthoringSession

**M8（必达，尚未实现）：**

- 每个 Worker / 角色展示执行模式：跟随已发布工作流，或直接执行（D18）
- 无 capability 时控件 disabled，并说明原因；禁止假 mode
- 已选模式在随后的 Task / Run 上只读可审计，不在执行中偷偷切换

空态：

- 无自定义团队：说明仍可使用预设 Software Development Team，并提供「新建团队」（M7 才可点成功）
- 库为空：说明可用 Chat 创建角色草稿，或从预设复制；不得假装已有商店可装
...
- 无云端组织、跨用户分享、Worker IM
- **现在不是一等面：** Marketplace / 机器人商店。不要画可点成功的商店；未来经库安装
- 不把只读预设页或成员三字段写成「角色版本库已完成」
- 不把库页接线写成卡片三字段已完成
...
**M7 必达（可视化画布 + 对话生成，尚未实现）：**

- 主编辑面是画布：创建草稿、拖拽/连接有限 DAG、保存、发布不可变版本
- 对话生成是同一循环的作者入口：用户描述「创建 bot1/bot2、跑流程 X、bot 负责任务 Y」，Agent **生成**草稿后必须能「在画布中编辑」，不能锁死
- 已发布版本只读；再编辑创建新 version
- 画布、目录与对话入口共用本导航：列表/详情可进入「对话生成」或「在画布中编辑」
- 高度可定制是本页产品要求：用户能改角色、步骤与边，而不是只能消费预设

空态：

- 无已发布工作流：说明可从预设复制、新建空白图，或对话生成草稿（M7）；M3 展示目录或诚实空列表
- 草稿未发布：明确写「未发布，Runtime 不会执行此图」
- 保存/发布/生成失败：保留画布或对话上下文，不假装已发布或已生成
- 对话入口在写接口未就绪时不得渲染可点击成功态

非目标：

- 不宣称 Mock 或真实 Runtime 执行未发布图，也不把目录接通写成「已可执行」
- 不把画布或对话当成通用 iPaaS / 现在可点的 Marketplace；节点类型以领域模型为限
- 不在画布上原地改活动执行 DAG（确认计划后的执行图仍按 D02 冻结）
- 不把「Agent 在对话里说做完了」写成 Task/Run 完成；对话只生成定义或读取事实

### 6.3 选择绑定工作流或直接执行（M8，尚未实现）

落点：AI 团队成员卡、Task 分配或 Run 启动面（不另开一级导航）。每个 Agent 做事前，用户看到并选择：

1. **跟随已发布工作流** — 只执行该项目已确认 `WorkflowVersion` 中轮到它的节点
2. **直接执行** — 按当前任务目标即席执行，不走该次已发布图

无 probe 支持时隐藏或 disabled，并返回明确错误。两种模式都不是「无 Policy 乱跑」。当前代码无此控件，不得预画可点成功态。

### 6.4 全局 Chat（V0.1 就要有；壳已接线，句意 / 空闲写卡 planned）

Chat 是**壳按钮**（任意页可开，路由 `/chat`），不是一级侧栏「某员工私聊」，也不是只钉在工作流作者页。壳已接线，不得再画「HTTP 未就绪的空壳」当现状。句意分类、空闲写卡未实现前，不得把关键词分类或气泡画成已完成。

按句意落到对象（分类结果不是完成；一句话多意图可分别落地，**不**强制三张确认卡）：

1. **建/改角色** — 确认 → 库中未发布草稿；不挂 `projectId`，不经 AuthoringSession；可跳「我的角色版本库」
2. **空闲说话写卡** — 写入谁 / 怎么干活 / 技能中的相应字段；已发布则 fork；不派 Task/Run，不开 IM
3. **请到项目** — 该 Project 的 Team 成员写入已发布 `workerVersionId`；缺项目先问
4. **创建流程** — 必须有 `projectId` → AuthoringSession → WorkflowDraft → 可去画布；未发布不执行
5. **问进度** — 只渲染已有 Task / Run / Event / Artifact；无记录说还没有；不出现无事实的「做完了」
6. **交流工作** — 执行中才挂 `runId` 走已有 Run 输入；没有挂点不发写。认不出再问，有项目也不得默成交流工作

仍禁止：Worker IM、无项目聊天室、聊天当完成、把「去做」画成已在跑（D18 未就绪则诚实失败）。空闲说话不是 direct。
