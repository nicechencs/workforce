...
**版本：** V0.1 Draft  
**状态：** Product UI baseline（§1 项目制主对象；§4.3 为项目详情现行规范；§4.6 / §6.2 服务项目循环；§6.3 T21 UI 切片已接线，M8 未完成）  
**日期：** 2026-09-11  
**修订：** 2026-09-12 — §6.3：T21 启动面诚实控件已接线；无 `orchestration.direct` 则禁用。不宣称 M8 完成。2026-09-11 — 补 D19：执行位置默认**本机**；远程与容器是一等 Placement，V0.1 不得伪造在线远程节点或可点成功的容器调度。同日产品主对象定为**项目制**（决策登记 §0）：围着一个 Project 编排 Team、Tasks、Workflow（含自定义 Team 与画布）。D15/D16 是该循环的必达环节，不是外挂。只读 `GET /workflows` 是已接通的 M3 过渡切片。同日补 D17 对话生成工作流、D18 按 Agent 双执行模式。§4.3.2：已发布执行图的公开 Task DTO 必须返回 `dependsOn`；UI 展示真实依赖边，字段缺失时才写“依赖：未返回”。2026-09-10 — §2：`P0`/`P1` 是交付切片深度，不是侧栏可见性；一级导航全部出现在左侧栏。§4.3 从“建议标签”改为现行条款，并消解与线框 §3 的冲突。
...
主循环：`Project → Team → Tasks → Workflow 编排 → 执行与验收`（[决策登记 §0](../planning/decision-register.md#0-产品模型项目制)）。画布、自定义 Team、对话生成与双执行模式是这条循环上的页面能力，不是与项目并列的独立产品。D17 会话 DTO **已冻结**，本机会话可追加用户消息，编排 Agent **尚未实现**；D18 字段已冻结，T21 项目启动面 UI 已探针/部分接线，Task 详情不挂未接线控件，M8 **未完成**。页面不得假成功。
...
产品**必须**能在项目循环里高度定制 Workflow：用对话生成草稿、用可视化画布/结构化面编辑，并用自定义 Team 给项目配团队（M7）。每个 Agent 执行时必须能诚实选择绑定已发布工作流或直接执行（M8）。M3 可用只读模板/预设作为过渡切片，不得再写成「V0.1 不做画布 / 对话生成 / 自定义 Team / 双执行后置」。未实现前不得渲染可点击成功态。
...
- 编辑已发布编排必须新建 version
- 对话生成（D17）可一并产出角色草稿，但仍须在本页或 Team 编辑面可改，再发布 TeamVersion
...
- 不把画布或对话当成通用 iPaaS / Marketplace；节点类型以领域模型为限
- 不在画布上原地改活动执行 DAG（确认计划后的执行图仍按 D02 冻结）
- 不把「Agent 在对话里说做完了」写成 Task/Run 完成；对话只生成定义

### 6.3 选择绑定工作流或直接执行（M8；T21 UI 切片已接线）
...
无 probe 支持时隐藏或 disabled，并返回明确错误。两种模式都不是「无 Policy 乱跑」。T21 启动面已有诚实控件：无 `orchestration.direct` 时 direct 不可点。不得把该切片写成 M8 完成或生产 Codex direct。
