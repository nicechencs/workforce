...
主循环：`Project → Team → Tasks → Workflow 编排 → 执行与验收`（[决策登记 §0](../planning/decision-register.md#0-产品模型项目制)）。画布、自定义 Team、对话生成与双执行模式是这条循环上的页面能力，不是与项目并列的独立产品。D17 会话 DTO **已冻结**，对话发送 / 编排 Agent **尚未实现**；D18 字段已冻结，T21 项目启动面 UI 已探针/部分接线，Task 详情不挂未接线控件，M8 **未完成**。页面不得假成功。
