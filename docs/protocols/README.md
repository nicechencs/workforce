...
`org_` `prj_` `tsk_` `run_` `wfd_` `wfv_` `wfi_` `wfn_` `tm_` `tmv_` `art_` `arv_` `apr_` `wsp_` `wsi_` `evt_` `op_` `ndl_` `rtm_` `usr_` `cli_` `cas_` `cam_`
...
- `v0.1/authoring-session.schema.json` — D17 对话会话 `AuthoringSessionDto` 与未发布草稿 `AuthoringDraftDto`。可执行源在 `packages/protocol`。V0.1 传输为 Desktop-local / in-process；Desktop 可按该 DTO 做本机会话存储 + 仅用户 append。**不是** Daemon chat 资源或公开 chat HTTP path，也不是编排 Agent / M7 完成
