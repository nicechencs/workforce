...
# 不宣称真窗口对话编排可用；#31 `31d669be` headed FAIL（reload 丢会话）后已补 localStorage，headed 未复测 PASS；编排 Agent 仍 planned
```
...
1. **Headed Electron 真窗口点击验收**：happy-dom / opt-in `executeJavaScript` helper **不能**代替人工。用 `pnpm --filter @workforce/desktop dev` 点目录对话框、SSE、Run 控制台、项目详情六标签与视觉。T18 画布、T19 草稿持久化与 T21 项目「开始执行」UI smoke（`f3b2045` / `WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`）已有各自 headed PASS，不代替本条其余路径，也不是完整 headed 套件，也不是 #28 透传 headed。  
...
6. **M7 对话生成工作流（T20）未完成**：Desktop 作者面壳与写 API 落草稿已接通（#24 `edce1a4`）；headed 作者面壳 **PASS**（`66a9c284`）。会话 / 草稿 DTO **已冻结**；V0.1 传输 **已冻结**为 Desktop-local / in-process（无 Daemon chat path）。**本机会话存储 + 仅用户 append 已接线**（`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该切片）；快照现写本机 `localStorage` 以跨 renderer reload。#31 `31d669be` 真窗 **FAIL**（SPA 导航 OK，Ctrl+R 丢会话）；reload 修复后 **headed 未复测 PASS**。**编排 Agent / 对话生成仍 planned**。不得把用户消息或结构化落草稿写成「对话编排已完成」，也不得渲染假 Agent 成功回复。
