...
修订：2026-09-12 — T21 Desktop **双执行模式 UI 切片**：项目/Task 启动面可选 `orchestrationMode`，`GET /capabilities` 未声明 `orchestration.direct` 时禁用 direct；现有 `POST /projects/{id}:start` 透传字段，无能力则 `422 unsupported_capability`。**不**宣称 M8 完成、headed PASS 或生产 Codex direct。#25 已 squash 进 main（`b700010`）：T02 冻结 D18 字段。T20 Desktop **UI shell + 写 API 落草稿**已在 main：会话协议未冻结时禁用发送（`CHAT_SESSION_PROTOCOL_FROZEN=false`）。作者面壳 headed **PASS**（`66a9c284`）。T18 headed create/edit/save/publish **PASS**（`fbe13dea`）与 T19 headed 草稿持久化 **PASS**（`406ee6d2`）保留。M7 **未完成**。D19 远程 enrollment 与容器 runner **未实现**。

## 1. 本轮目标与结果

目标：跑通 **M3 Mock 完整流程**（规划文档 §5），并行补齐 Daemon 真实用例、Electron/React 壳与 P0 页面。产品主对象是 **Project（项目制）**：M3 用预设 Team + 只读工作流目录走完一个项目闭环。M7 **写 API + 协议**已接通（#23 `15c058f`）。T18 画布 UI 已接线且草稿保存走同一套写 API；headed 真窗 create/edit/save/publish 已 **PASS**（`fbe13dea`，main `6eeea107`）。自定义 Team（T19）**桌面写面 UI + 草稿持久化已接线**（main `0e2a156`）；headed 草稿 save→reload **PASS**（`406ee6d2`）。T20 只有 Desktop **作者面壳 + 写 API 落草稿**，对话会话协议仍 **planned**（chat 禁用至 session DTO）。按 Agent 双执行模式是 **M8 已规划、未完成**：T02 已冻结 `orchestrationMode`；T21 **Desktop UI 切片已接线**（happy-dom 选择/禁用；headed 未跑）。D19 Placement（本机默认；远程与容器）**产品模型已冻结、实现未完成**。不得把 T18 / T19 / T20 壳写成 M7 或对话编排完成，也不得发明 chat / `:direct` / enrollment endpoint，不得宣称生产 Codex 已支持 direct。happy-dom / proxy 仍不能代替其余真窗路径（目录对话框、SSE、Run 控制台）。
...
| ID | 状态 | 证据 |
|---|---|---|
| T00 | 完成（项目制 + M7/M8 + D19 Placement 决策已补写） | M0–M3 冻结仍有效；§0 项目制；D15/D16 画布与自定义 Team；D17 对话生成（M7 planned，仅有 UI shell）；D18 双执行模式（M8 planned；`orchestrationMode` 字段已冻结；T21 UI 切片已接线，M8 未完成）；D19 本机默认、远程与容器为一等 Placement。M7 **写 API + 协议**已接通。T18 画布 UI **已接线**，headed 真窗 create/edit/save/publish **PASS**（`fbe13dea`）。T19 桌面写面 + 草稿持久化 **已接线**，headed save→reload **PASS**（`406ee6d2`）。**未实现**会话协议 / 编排 Agent（T20 剩余）、M8 调度 / 生产 Codex direct、远程 enrollment 与容器 runner。M7 未完成 |
...
| T21 | Desktop UI 切片已接线（M8 未完成） | 双执行模式（D18）。`orchestrationMode` **已冻结**在 protocol `StartRunRequest`。项目详情 / Task 详情有诚实选择面：`GET /capabilities` 未声明 `orchestration.direct` 时 direct **disabled**；选择写入现有 `POST /projects/{id}:start`，无能力则 `422 unsupported_capability`。happy-dom 覆盖选择与禁用。**无**新 Run path。**未跑** headed。**不**宣称 M8 调度完成或生产 Codex direct |
...
7. **M8 双执行模式未完成**：T21 **UI 切片已接线**（选择面 + capability 禁用 + 现有 `:start` 透传）。`GET /capabilities` 现声明 `orchestration.direct=false`，因此 direct 不可点成功。调度仍只走已发布图（缺省 `workflow_bound`）。headed **未跑**。不得宣称 M8 完成或生产 Codex 已验证 direct。
