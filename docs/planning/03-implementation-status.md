...
M7/M8 是 V0.1 release gate，**不是**当前 M3 通过条件，也**尚未完成**。2026-09-12 起仓库里**已有**部分 M7/M8 切片代码（见 T18–T21），不得再写「都还没有代码」。M7 完成仍需 T20 真窗口 allowlist、Codex 编排 Agent 生成草稿、Task patch/失败恢复、自定义 Team 发布/绑定闭环；Host transient prompt handoff 与 typed send HTTP **已有**，不等于对话编排完成。T19 写 UI headed 已 PASS，不等于 M7 完成。M8 完成仍需 `direct` 调度与 Daemon `POST /tasks/{id}/runs`。T21 项目详情 start headed 已 PASS，不等于 M8 完成。D17/D18 前置：T02 已冻结执行三轴（`packages/protocol/src/execution.ts`），T04 已落地 `005`–`009`（含 `009_workflow_authoring_scopes`），S2a 已落地 `SqliteProjectExecutionSnapshotRepository`（insert-once）。受管 Run 启动现构造 `RunExecutionSnapshot` 并经 world snapshot 投影。**仍未实现**：backfill / switch / contract、T16 current-M3 upgrade fixture、Daemon `POST /tasks/{id}/runs`、ad-hoc direct 调度。
...
| T10 | **composition + 作者会话 HTTP** | 生产 `main()` 用真实服务；`taskDto()` 填公开 `dependsOn`；Mock 产物经 `LocalArtifactStore` `register`。AuthoringSession HTTP 已注册：`POST /projects/:id/authoring-sessions`、`GET /authoring-sessions`、`POST .../messages`、`.../turns/:id/_cmd/confirm`（及 cancel/retry/close）；typed client 已接线。SSE/`listEvents` 仍过滤内存 `world.events`。**无** `POST /tasks/{id}/runs`。测试默认 Fake 仍绿 |
