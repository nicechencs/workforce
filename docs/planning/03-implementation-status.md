...
修订：2026-09-13 — WV-T00-ALIGN：按 `d376ba2` 源码对齐。卡片三字段、句意分类、空闲写卡与 Team `resolveTeamDraft` **已接线**。Task `resolveTaskPatch` **未接**（`confirmChatDeps` 无该 resolver、无 `tasks` port）。Daemon **有** `POST /tasks/:id/runs` 路由与 typed `startTaskRun`，但 Electron allowlist 仍 `blockedUntil: "T10-TASK-RUN-HTTP"`；`GET /capabilities.orchestration.direct === false`；composition `startTaskRun` 硬编码 `requireWorkflowBinding: true`；Chat「去做」仍 `unsupported_capability`。T08 `ArtifactEvaluator` 已 `waitCapturedExit`；`taskCompletionBarrier` 在**零条 evaluation** 时仍放行 `completed`。Windows Job Object `wait()` 已在 `packages/process`（capture 不可用 fail-closed）。不发明 HTTP path。**不**宣称 M7/M8 完成、headed 或 Marketplace 当前债。

修订：2026-09-13 — T00-DOCS：按 `e142f72` 源码把**我的角色版本库** HTTP/UI 与**全局 Chat 壳**标成**已接线**（`GET/POST /workers*`、`POST /chat-intents:classify`、问进度投影、Team `workerVersionId`、allowlist 已放行、侧栏库页、壳上 Chat）。卡片三字段（他是谁 / 怎么干活 / 技能）、按句意分类、空闲说话写卡仍 **planned**。分类仍是关键词；未识别且带 `projectId` 会落到 `discuss_work`。`WorkerVersionDto` 无卡片列。Daemon `confirmChatDeps` 无 `resolveTeamDraft` / `resolveTaskPatch`。direct 仍诚实失败。不发明 HTTP path。**不**宣称 M7/M8 完成、headed 或 Marketplace 当前债。

修订：2026-09-13 — T00-DOCS（前次）：按已确认方向把**我的角色版本库**、**全局 Chat 壳**、Team 成员目标 `workerVersionId`、Marketplace「现在不是一等面、未来要做」写入规划。当时本页把库页、全局 Chat、问进度投影、`workerVersionId` 写入标成 **planned**；`e142f72` 源码已接线，见上条。
...
M7/M8 是 V0.1 release gate，**不是**当前 M3 通过条件，也**尚未完成**。2026-09-12 起仓库里**已有**部分 M7/M8 切片代码（见 T18–T21），不得再写「都还没有代码」。角色库页与全局 Chat 壳 **已接线**，不等于 M7 完成。卡片三字段、句意分类、空闲写卡与 Team `resolveTeamDraft` **已接线**，不等于 M7 完成。M7 完成仍需 Codex 编排 Agent 生成草稿、Task patch/失败恢复、自定义 Team 发布/绑定闭环；Host transient prompt handoff 与 typed send HTTP **已有**，不等于对话编排完成。T19 写 UI headed 已 PASS，不等于 M7 完成。M8 完成仍需 ad-hoc `direct` 产品落点（Chat「去做」→ 项目内 Task → `POST /tasks/{id}/runs`）。Daemon **已有**该 HTTP 路由，不要再写成「无路由」。T21 项目详情 start headed 已 PASS，不等于 M8 完成。D17/D18 前置：T02 已冻结执行三轴（`packages/protocol/src/execution.ts`），T04 已落地 `005`–`016`（含 `009_workflow_authoring_scopes`、`015_worker_library`、`016` 卡片三列），S2a 已落地 `SqliteProjectExecutionSnapshotRepository`（insert-once）。受管 Run 启动现构造 `RunExecutionSnapshot` 并经 world snapshot 投影。**仍未实现**：backfill / switch / contract、T16 current-M3 upgrade fixture、allowlist 放行 Task Run、`orchestration.direct` probe、ad-hoc direct 调度、Task `resolveTaskPatch`、evaluation 完成门（零条 evaluation 仍可 `completed`）。
...
| T02 | 完成（M3 字段 + 图/Authoring 契约 + V0.1 Schema 门禁 + HTTP DTO） | `packages/protocol` 公开 `TaskDto` / `RunDto` / `ProjectDto` / `TeamDto`；daemon `modules/dto.ts` 从 protocol 再导出，不是第二套手写副本。`dependsOn` 是公开契约。`WorkflowGraphDefinition` 是严格有限 DAG；AuthoringSession / Message / Turn、`startTaskRunInputSchema` 与 `startDirectTaskRunInput/Accepted` 已冻结。显式 Zod registry 生成当前 **31** 个公开 JSON Schema（`docs/protocols/v0.1/*.schema.json`），`protocol:schema:check` 阻止漂移。OpenAPI 和未登记协议面仍未实现。HTTP `POST /tasks/{id}/runs` 的 schema **与 Daemon 路由已有**；Electron allowlist 仍阻断，`orchestration.direct` probe 为 false，不是产品落点 |
...
| T04 | M3 持久化完成；D17/D18 为 expand + Run 快照投影 + 审计分类 + 写入侧对账 + D15 不可变版本 + authoring repository + 009 scopes + **015 角色库表** + **016 卡片三列** | migration **001–016** + entity repos；重启以 SQLite 实体表为准。`005_execution_axes_expand` 未改号（drafts / change_sets / snapshots + 可空轴列）；`006_catalog_definitions` 为 catalog 四表；`007_runtime_profile_transport_expand` 新增可空、受约束的 profile transport；`008_execution_axis_migration_audit` 是无 Run FK、只存摘要的 append-only 历史分类账本，**不会**回填或提升记录；`009_workflow_authoring_scopes` 约束 catalog workflow 的 Project/organization 授权；`015_worker_library` 为 `catalog_workers` / `catalog_worker_versions` / `worker_drafts`（列表/草稿/发布 insert-once / 归档/fork）；`016` 为 published/draft 的 `who` / `how` / `skills`。真实 `workflow_versions` 使用稳定 JSON SHA-256 insert-once，实例从版本读图；仅旧空 FK placeholder 可一次升级。workflow/team 草稿 append-only revision CAS；ChangeSet 与 steps 同事务持久化。Daemon chat confirm 已接线 `workflowAuthoringScopes`。新 sidecar 仅在 SQLite commit 后发布，raw constraint/CAS conflict fail-fast；旧 sidecar 不自动修复。数据库 Run repository / world snapshot 能严格校验、原子读写完整执行快照；Application `startRun` 现构造该快照并经投影落列。旧历史行在 backfill 前仍可为空轴。**无** backfill / switch / contract |
| T05 | 部分：Runtime/Mock/Daemon Proposal 输出链 + transient handoff | `runtime.authoring.proposal` 是唯一结构化输出；Mock fixture、Host 入库前 parser 清洗/摘要替换、audit-only 与非 Authoring Run 拒绝、composition 从 handle 反查 Run/Project 后调用 Application `recordAuthoringProposal` 均已接线。Host `startWithInitialInput` 做一次性 transient prompt handoff，磁盘只存 digest；无可信交接则 fail-closed。Codex 在 authoring `snapshotRef` 上于 spawn 前返回 `unsupported_capability`（`authoring.proposal`）。`StartRunRequest` 仍不携带 raw intent。不得把 `authoring.start` 误报为 Agent 已收到输入。真实 Codex JSONL 映射、Task patch 仍未完成。Mock `authoring-proposal` 现出 `workflow` \| `team`，**无** `targetType: "task"` |
| T06 | POSIX + Windows captured-process 终态切片已实现；capture 不可用仍 fail-closed | `CapturedProcess.wait()` 在 POSIX 上等待 root exit、stdout/stderr EOF 与已验证的受管进程组终结；Windows 走 Job Object `wait()`（`packages/process`）。output overflow、提前放弃、截断关闭、I/O、取消与不可验证树均映射 `ProcessControllerError`，不伪造 exit result。Job / capture 不可用时继续 fail-closed。macOS/Linux 真机与 Windows 真机 Job 证据仍未在本 tip 宣称 |
...
| T08 | 完成库并接入 Mock composition（本切片权威） | 所有权仍是 `packages/artifacts`（不是 Daemon 私有第二套规则）。composition 以 `LocalArtifactStore` 为 Mock 产物字节/元数据权威；公开 content 读精确 `artifactVersionId`。`ArtifactEvaluator` 已 `waitCapturedExit`。**不是**未开始，也**不是** world.json / `bodyBase64` 权威。composition `recordRunSucceeded` **不**把 T08 结论写入 `world.evaluations`；零条 evaluation 时 T09 完成门仍放行 `completed` |
| T09 | M3 用例 + normalized dependency + D02 snapshot/start + Run placement/lease 切片 | `m3-path.test.ts`；Daemon 已调用 `WorkforceApp`。confirm-plan 只冻结 snapshot 并进入 `ready`，`:start` 才从 snapshot graph 建实例/任务；SQLite/world restart 先恢复 canonical graph 再恢复 snapshot。`SqliteWorldSnapshot` 在所有 Task 落行后同步 `task_dependencies`，以 `required_status` 保留普通依赖并替换过时边。受管 Run 启动走 Placement Scheduler，事务内写 lease/`RunExecutionSnapshot`，提交后再 spawn，Host 上每个 Run 有独立 fencing。当前仍未切到已发布 catalog graph、没有真实 policy snapshot、ad-hoc `direct` 产品落点或远程/分布式 placement。`taskCompletionBarrier` 只拦 `verdict=fail`，**零条 evaluation 仍 `ok: true`** |
| T10 | **composition + 作者会话 HTTP + 角色库 / Chat classify / 进度 + Task Run 路由壳** | 生产 `main()` 用真实服务；`taskDto()` 填公开 `dependsOn`；Mock 产物经 `LocalArtifactStore` `register`。AuthoringSession HTTP 已注册：`POST /projects/:id/authoring-sessions`、`GET /authoring-sessions`、`POST .../messages`、`.../turns/:id/_cmd/confirm`（及 cancel/retry/close）；typed client 已接线。角色库 HTTP 已注册（`GET/POST /workers*`、drafts、`:publish` / `:archive` / `:fork`）。`POST /chat-intents:classify` 与 `GET /projects/{id}/progress` 已接线。**有** `POST /tasks/:id/runs`（`startTaskRun`）；typed `DesktopClient.startTaskRun` 已有。SSE/`listEvents` 仍过滤内存 `world.events`。`confirmChatDeps` **有** `resolveTeamDraft` + `teamIdentities` / `teamDrafts`，**无** `resolveTaskPatch`、**无** `tasks` port。`startTaskRun` 硬编码 `requireWorkflowBinding: true`。`capabilities.orchestration.direct === false`。测试默认 Fake 仍绿 |
| T11 | **本轮完成壳；源码入口 Node preflight 已接线；设计系统收口画布/Team** | Electron + Vite + React + IPC + feature glob；2026-09-11 补齐设计系统：`packages/ui/src/tokens.ts` / `theme.ts`（四档字号、8/12/16 圆角、浅深主题、5 主题色、8 浅色画布、Agent 色槽）、`renderer/styles.css` 语义 class 层、`renderer/components/` 基础组件与图标、`renderer/app/theme.tsx` 主题提供者与首屏 `bootstrapTheme()`。2026-09-12 补 Dialog / DropdownMenu / Tooltip / Toast / Table / Skeleton；画布与 Team 写页改组合这些组件；`features/projects/ui.ts` 不再持有第二套 hex，只再导出 `--wf-*` 供作者壳 / orchestration 过渡。Supervisor 仅对 TypeScript 源码入口执行 Node ≥22.7 preflight；stderr pipe 已撤回。崩溃/端口冲突的结构化诊断、运行期诊断导出和三平台证据仍 planned。对齐 AgentHub 视觉基线，实现栈刻意不同（无 Tailwind/Radix/lucide 依赖），差异见 [UI 设计系统](../product-ui/04-design-system.md) §7 |
...
| T19 | 写 UI + 草稿 persist + `workerVersionId` + **卡片三字段已接线**；headed 真窗 **PASS**；M7 未完成 | 换掉只读 `rejectCustomTeamSave()` 桩。列表/详情走 `GET /teams` + `GET /teams?status=draft` + `GET /teams/{id}`；保存走已有 `POST/PATCH /teams` 与 versions。发布按钮诚实：无假 publish/bind 成功。项目 Settings 可选手动绑定已发布自定义 TeamVersion（须服务端回传 `teamVersionId`）。成员写入目标是已发布 `workerVersionId`（读仍兼容旧三字段）。预设 Team 三个已发布 WorkerVersion。库/选择器可见 `who` / `how` / `skills`。headed 真窗口 **PASS**（PR #34 head `06e6b659`，报告 `/workspace/qa-issues/WORKFORCE-PR34-06e6b659-T19-T21-TRUEWINDOW.md`；已 squash 进 `dev` `ae0f4e6`）。本 tip 卡片未 headed。**不**宣称 M7 完成 |
| T20 | **全局 Chat 壳已接线**；`CHAT_SESSION_PROTOCOL_FROZEN=true`；typed send 已接线；**句意分类 / 空闲写卡已接线** | `features/chat/` 为壳上全局入口；`workflow-authoring` 仍可由工作流页 `?authoring=1` 挂入。Chat 确认 `POST /workers` 进库（可无 `projectId`，不经 AuthoringSession）。问进度走只读投影；交流工作挂 `projectId`/`runId` 才 `POST /runs/{id}:input`。空闲说话写卡（草稿 PATCH / 已发布 fork）。「去做」disabled + `unsupported_capability`。Electron allowlist 已放行 workers / `chat-intents:classify` / progress / authoring-sessions；**未**放行 `POST /tasks/{id}/runs`。无 Codex 编排 Agent。本 tip 库/Chat **未** headed。禁止假 Agent 完成。**不是** D18 direct 落点，**不**宣称对话编排或 M7 完成。 |
| T20-B | Proposal 生命周期 + staged-apply + chat confirm（Workflow + **Team**）+ Host/Daemon handoff 骨架 | `authoring.start` 建受治理 Task/Run；raw intent 不进 `StartRunRequest`。Daemon `setInitialInput` + Host `startWithInitialInput` 做一次性 transient handoff，磁盘只存 digest；重启不可恢复须 fail-closed。Runtime SPI/Mock 的 Proposal 由 handle 反查后生成 ChangeSet；`confirmAuthoringChatProposal` 已接 workflow create/update 与 Team `resolveTeamDraft`。Application 已有 `landChatTaskPatch` / `resolveTaskPatch`；Daemon `confirmChatDeps` **无** `resolveTaskPatch`、**无** `tasks` port。turn retry 为 `unsupported_capability`。SQLite 只存会话 ref/hash/脱敏 preview。Task patch、部分失败恢复、Codex 映射与 UI 诚实失败展示仍未收口 |
| T21 | 项目详情已挂 mode 控件；`:start` 回显；受管 Run 现写执行快照；headed 真窗 **PASS**；M8 未完成 | `GET /capabilities.orchestration`；`:start` 可选 `orchestrationMode`；`direct` probe 为 false → 控件禁用 / 422。项目「开始执行」把控件选中的 mode 传入 `startProject`。composed `startProject` 把 mode 传入 `app.start()` 并回显 DTO。Application `startRun` 现构造 `RunExecutionSnapshot`（含 `orchestrationMode`）并经 SQLite world snapshot 投影落列。**不**写入 protocol `StartRunRequest`。Daemon **有** `POST /tasks/:id/runs`，allowlist 仍 `blockedUntil: "T10-TASK-RUN-HTTP"`。`:start` 的 `direct` **仍走已发布图**，不创建 ad-hoc Task。headed 真窗口 **PASS**（#34 / `06e6b659` → `ae0f4e6`）。无 Codex direct、无 ad-hoc `direct` 调度、**不**宣称 M8 完成 |
...
壳导航按更正后的 [IA §2](../product-ui/01-information-architecture.md)：**P0/P1 是切片深度，一级导航全部 `primary`**。IA 工作流用户目的现为「查看、编辑和发布可复用工作流（含对话生成与画布）」。**当前代码**：只读目录仍接通；另有画布壳、catalog 写 API、自定义 Team 写 UI、**角色库页**、**全局 Chat 壳**、作者壳、项目详情 orchestrationMode 控件与 composed passthrough（见 T18–T21）。T19/T21 headed 真窗已 **PASS**（#34 / `06e6b659` → `ae0f4e6`），不得把该 PASS 写成 M7/M8 完成、T18 画布 headed、T20 D18 direct 落点或 `direct` 调度。不得宣称 Mock/Codex Runtime 可执行未发布定义。远程/容器 Placement 是产品冻结，runner **未实现**。
...
5. **T19 剩余**：写 UI + 草稿 persist + `workerVersionId` + 卡片三字段已在本 tip。headed 真窗 **PASS**（`06e6b659` / `ae0f4e6`，同上报告）。发布/绑定不得假成功；未发布草稿仍不可开始规划。**角色库页已接线**。不等于 M7 完成。  
6. **T20/T20-B 剩余**：全局 Chat 壳、`POST /workers` 进库、Daemon AuthoringSession HTTP、typed send、Host transient handoff、Workflow/Team 聊天确认（`resolveTeamDraft`）、句意分类与空闲写卡已在本 tip；`CHAT_SESSION_PROTOCOL_FROZEN=true`；allowlist 已放行 workers / classify / progress / authoring-sessions。**仍缺** Task `resolveTaskPatch`（composition 无 resolver / `tasks` port）、T20 headed、Codex 编排 Agent、部分失败恢复与 turn retry。Chat「去做」仍 `unsupported_capability`。不得把壳写成 Agent 已完成或 M7 完成。  
7. **T21 / T09-DIRECT / T10-TASK-RUN-HTTP 剩余**：控件已挂项目详情；`:start` 回显与 capability gating 已有；受管 `startRun` 现构造 `RunExecutionSnapshot` 并经投影落列。headed 真窗 **PASS**（同上报告）。Daemon **有** `POST /tasks/:id/runs`；allowlist 仍 `blockedUntil: "T10-TASK-RUN-HTTP"`；`orchestration.direct === false`；composition `requireWorkflowBinding: true` 会拒 ad-hoc；无 ad-hoc `direct` 调度。**禁止**把 mode 写入 `StartRunRequest`。无 Codex direct。空闲说话不是 direct。**不**宣称 M8 完成。  
...
10. T08 `ArtifactEvaluator` 已 `waitCapturedExit`；Windows Job `wait()` 已在 `packages/process`（capture 不可用 fail-closed）。composition `recordRunSucceeded` 不把 T08 结论写入 `world.evaluations`；`taskCompletionBarrier` 在**零条 evaluation** 时仍放行 `completed`。缺产物或没有 **pass** 的完成门仍缺。不得用 private `inspect()` 扩展或 fake adapter 伪造通过。
11. **Marketplace：** 现在不是一等面；当前无商店 UI 符合「现在不做」。未来要做，不要写成永久禁止，也不要写成当前债。
