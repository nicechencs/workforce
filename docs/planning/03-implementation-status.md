...
M7/M8 是 V0.1 release gate，**不是**当前 M3 通过条件，也**尚未完成**。2026-09-12 起仓库里**已有**部分 M7/M8 切片代码（见 T18–T21），不得再写「都还没有代码」。角色库页与全局 Chat 壳 **已接线**，不等于 M7 完成。M7 完成仍需句意分类、卡片三字段、空闲写卡、Codex 编排 Agent 生成草稿、Task patch/失败恢复、自定义 Team 发布/绑定闭环；Host transient prompt handoff 与 typed send HTTP **已有**，不等于对话编排完成。T19 写 UI headed 已 PASS，不等于 M7 完成。M8 完成仍需 `direct` 调度与 Daemon `POST /tasks/{id}/runs`。T21 项目详情 start headed 已 PASS，不等于 M8 完成。D17/D18 前置：T02 已冻结执行三轴（`packages/protocol/src/execution.ts`），T04 已落地 `005`–`015`（含 `009_workflow_authoring_scopes`、`015_worker_library`），S2a 已落地 `SqliteProjectExecutionSnapshotRepository`（insert-once）。受管 Run 启动现构造 `RunExecutionSnapshot` 并经 world snapshot 投影。**仍未实现**：backfill / switch / contract、T16 current-M3 upgrade fixture、Daemon `POST /tasks/{id}/runs`、ad-hoc direct 调度、卡片三列。
...
| T02 | 完成（M3 字段 + 图/Authoring 契约 + V0.1 Schema 门禁 + HTTP DTO） | `packages/protocol` 公开 `TaskDto` / `RunDto` / `ProjectDto` / `TeamDto`；daemon `modules/dto.ts` 从 protocol 再导出，不是第二套手写副本。`dependsOn` 是公开契约。`WorkflowGraphDefinition` 是严格有限 DAG；AuthoringSession / Message / Turn、`startTaskRunInputSchema` 与 `startDirectTaskRunInput/Accepted` 已冻结。显式 Zod registry 生成当前 **31** 个公开 JSON Schema（`docs/protocols/v0.1/*.schema.json`），`protocol:schema:check` 阻止漂移。OpenAPI 和未登记协议面仍未实现。HTTP `POST /tasks/{id}/runs` 的 schema 已有，Daemon 路由未实现 |
...
| T19 | 写 UI + 草稿 persist + `workerVersionId` 已接线；headed 真窗 **PASS**；M7 未完成；**角色库页已接线；卡片三字段 planned** | 换掉只读 `rejectCustomTeamSave()` 桩。列表/详情走 `GET /teams` + `GET /teams?status=draft` + `GET /teams/{id}`；保存走已有 `POST/PATCH /teams` 与 versions。发布按钮诚实：无假 publish/bind 成功。项目 Settings 可选手动绑定已发布自定义 TeamVersion（须服务端回传 `teamVersionId`）。成员写入目标是已发布 `workerVersionId`（读仍兼容旧三字段）。预设 Team 三个已发布 WorkerVersion。headed 真窗口 **PASS**（PR #34 head `06e6b659`，报告 `/workspace/qa-issues/WORKFORCE-PR34-06e6b659-T19-T21-TRUEWINDOW.md`；已 squash 进 `dev` `ae0f4e6`）。**不是**卡片三字段完成，选择器还看不见谁 / 怎么干活 / 技能，**不**宣称 M7 完成 |
| T20 | **全局 Chat 壳已接线**；`CHAT_SESSION_PROTOCOL_FROZEN=true`；typed send 已接线；分类仍是关键词；**句意分类 / 空闲写卡 planned** | `features/chat/` 为壳上全局入口；`workflow-authoring` 仍可由工作流页 `?authoring=1` 挂入。Chat 确认 `POST /workers` 进库（可无 `projectId`，不经 AuthoringSession）。问进度走只读投影；交流工作挂 `projectId`/`runId` 才 `POST /runs/{id}:input`。「去做」disabled + `unsupported_capability`。`classifyChatIntent` 只匹配「创建角色 / 创建流程 / 进度 / 交流」等字面；未识别且带 `projectId` 落到 `discuss_work`。Electron allowlist 已放行 workers / `chat-intents:classify` / progress / authoring-sessions。无 Codex 编排 Agent。本 tip 库/Chat **未** headed。禁止假 Agent 完成。**不是**句意落地或空闲写卡完成，**不**宣称对话编排或 M7 完成。 |
| T20-B | Proposal 生命周期 + staged-apply + chat confirm（仅 Workflow）+ Host/Daemon handoff 骨架 | `authoring.start` 建受治理 Task/Run；raw intent 不进 `StartRunRequest`。Daemon `setInitialInput` + Host `startWithInitialInput` 做一次性 transient handoff，磁盘只存 digest；重启不可恢复须 fail-closed。Runtime SPI/Mock 的 Proposal 由 handle 反查后生成 ChangeSet；`confirmAuthoringChatProposal` 仅 workflow create/update。`targetType !== "workflow"` 与 `step.targetType === "task"` 仍 not implemented；turn retry 为 `unsupported_capability`。Daemon `confirmChatDeps` **无** `resolveTeamDraft` / `resolveTaskPatch`。SQLite 只存会话 ref/hash/脱敏 preview。句意分类、空闲写卡、Team 聊天确认、部分失败恢复、Codex 映射与 UI 诚实失败展示仍未收口 |
...
5. **T19 剩余**：写 UI + 草稿 persist + `workerVersionId` 已在本 tip。headed 真窗 **PASS**（`06e6b659` / `ae0f4e6`，同上报告）。发布/绑定不得假成功；未发布草稿仍不可开始规划。**角色库页已接线**；**卡片三字段 / 请来时带卡片仍 planned**。不等于 M7 完成。  
6. **T20/T20-B 剩余**：全局 Chat 壳、`POST /workers` 进库、Daemon AuthoringSession HTTP、typed send、Host transient handoff 与 Workflow 聊天确认已在本 tip；`CHAT_SESSION_PROTOCOL_FROZEN=true`；allowlist 已放行。**仍缺**句意分类（现为关键词；未识别带 `projectId` 落到 `discuss_work`）、空闲说话写卡、卡片三字段、T20 headed、Codex 编排 Agent、Task patch、Team 聊天确认（`confirmChatDeps` 无 `resolveTeamDraft`）、部分失败恢复与 turn retry。不得把壳或关键词分类写成句意落地完成、Agent 已完成或 M7 完成。  
7. **T21 剩余**：控件已挂项目详情；`:start` 回显与 capability gating 已有；受管 `startRun` 现构造 `RunExecutionSnapshot` 并经投影落列。headed 真窗 **PASS**（同上报告）。Daemon **无** `POST /tasks/{id}/runs`；无 ad-hoc `direct` 调度。**禁止**把 mode 写入 `StartRunRequest`。无 Codex direct。空闲说话不是 direct。**不**宣称 M8 完成。  
...
11. **角色卡片三字段（planned）：** `WorkerVersionDto` / 草稿 / 库详情仍只有 `name` / `description` / `role` / 可选 `runtimeProfileId`。产品必印他是谁 / 怎么干活 / 技能；Runtime / Policy 不是必印。不得把库页接线写成卡片完成。
12. **句意分类与空闲写卡（planned）：** `classifyChatIntent` 仍是关键词；认不出不得默成交流工作。空闲对角色说话应写入卡片相应字段、已发布则 fork，**不**新建 Task/Run、**不**开 IM。当前不会写卡。
13. **Marketplace：** 现在不是一等面；当前无商店 UI 符合「现在不做」。未来要做，不要写成永久禁止，也不要写成当前债。
