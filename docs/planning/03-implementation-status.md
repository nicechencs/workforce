...
修订：2026-09-12 — T20 #31 tip `c66eebe8` 真窗复测 **PASS**（`WORKFORCE-PR31-c66eebe8-AUTHORING-SESSION-RETEST.md`：Ctrl+R 后同一会话 id + 用户消息仍在）。先前 `31d669be` 真窗 **FAIL**（reload 丢会话）。`CHAT_SESSION_PROTOCOL_FROZEN=true` 仍只表示本机用户 append + 本地存储，**不**宣称编排 Agent 或 M7。同日更早：本机会话快照改为 **localStorage**。同日更早：T20 Desktop-local 会话存储 + 仅用户 append 已接线。**不**发明 chat path / Daemon chat 资源。同日更早：D17 V0.1 会话传输冻结为 **Desktop-local / in-process**（对照已冻结 `AuthoringSessionDto` / `AuthoringDraftDto`）。同日 T21 / D18 **composed `:start` 透传**：`orchestrationMode` 不再只在 HTTP 边校验后丢掉；`startProject` 写入现有 `app.start` / `project.start` receipt、`ProjectRecord`、后续 `startRun` / `StartRunHostRequest` / `StartRunRequest` 与 `RunDto`。`orchestration.direct=false` 时 direct 仍 `422 unsupported_capability`。调度仍只走已发布图，**没有** direct scheduler。#26 `f3b2045` 项目「开始执行」UI / start path headed smoke **PASS**（`WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`：direct 诚实 disabled；`workflow_bound` 能启动 Run）。#28 composed 透传仅 unit / composed 测试，本切片**未跑** headed。**不是**完整 headed 套件，**不**宣称 M8 完成或生产 Codex direct。**Task 详情不挂未接线控件**。#27 已 squash 进 main（`4274bb2`）。#25 已 squash 进 main（`b700010`）。T20 Desktop **UI shell + 写 API 落草稿**已在 main。作者面壳 headed **PASS**（`66a9c284`）。T18 / T19 headed PASS 保留。M7 **未完成**。D19 远程 enrollment 与容器 runner **未实现**。

## 1. 本轮目标与结果

目标：跑通 **M3 Mock 完整流程**（规划文档 §5），并行补齐 Daemon 真实用例、Electron/React 壳与 P0 页面。产品主对象是 **Project（项目制）**：M3 用预设 Team + 只读工作流目录走完一个项目闭环。M7 **写 API + 协议**已接通（#23 `15c058f`）。T18 画布 UI 已接线且草稿保存走同一套写 API；headed 真窗 create/edit/save/publish 已 **PASS**（`fbe13dea`，main `6eeea107`）。自定义 Team（T19）**桌面写面 UI + 草稿持久化已接线**（main `0e2a156`）；headed 草稿 save→reload **PASS**（`406ee6d2`）。T20 已有 Desktop **作者面壳 + 写 API 落草稿 + 本机会话存储 / 仅用户 append**，会话 / 草稿 DTO **已冻结**，V0.1 传输 **Desktop-local / in-process**（无 Daemon chat）；`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该本机切片。编排 Agent / 对话生成仍 **planned**。按 Agent 双执行模式是 **M8 已规划、未完成**：T02 已冻结 `orchestrationMode`；T21 **项目启动面 UI 已探针**（#26 `f3b2045` headed smoke **PASS**：`WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`），requested mode **已写入** composed start / Run / host 启动记录（#28 透传仅 unit / composed，未跑 headed；Task 详情不挂未接线控件；无 direct 调度）。D19 Placement（本机默认；远程与容器）**产品模型已冻结、实现未完成**。不得把 T18 / T19 / T20 壳写成 M7 或对话编排完成，也不得发明 chat / `:direct` / enrollment endpoint，不得宣称生产 Codex 已支持 direct。happy-dom / proxy 仍不能代替其余真窗路径（目录对话框、SSE、Run 控制台）。

**HTTP Mock 闭环已通过（headless）。** 桌面项目页有 happy-dom 点击 driver（默认 `pnpm test`）；这不是完整真窗口。T18 画布 create/edit/save/publish、T19 草稿 save→reload、T20 作者面壳（`66a9c284`）与 T20 本机会话 reload（`c66eebe8`）已有 headed **PASS**；目录对话框、SSE、Run 控制台与项目详情六标签的真窗验收仍需要。不得把 T20 headed 会话存储写成对话生成可用。Codex **未**做 live `exec`。

## 2. 任务状态（对照实现，不是旧清单）

| ID | 状态 | 证据 |
|---|---|---|
| T00 | 完成（项目制 + M7/M8 + D19 Placement 决策已补写） | M0–M3 冻结仍有效；§0 项目制；D15/D16 画布与自定义 Team；D17 对话生成（M7 planned；会话 DTO 与 Desktop-local 传输已冻结；本机会话存储 + 仅用户 append 已接线，编排 Agent 未接线）；D18 双执行模式（M8 planned；`orchestrationMode` 字段已冻结；T21 项目启动面已探针（#26 `f3b2045` headed smoke PASS）+ composed start 透传（#28 仅 unit/composed），M8 未完成）；D19 本机默认、远程与容器为一等 Placement。M7 **写 API + 协议**已接通。T18 画布 UI **已接线**，headed 真窗 create/edit/save/publish **PASS**（`fbe13dea`）。T19 桌面写面 + 草稿持久化 **已接线**，headed save→reload **PASS**（`406ee6d2`）。**未实现**编排 Agent / 对话生成（T20 剩余）、M8 调度 / 生产 Codex direct、远程 enrollment 与容器 runner。M7 未完成 |
| T01 | 完成 | pnpm + turbo monorepo；本轮补了 Electron/React/Vite lockfile |
| T02 | 完成（M3 字段）+ M7 写契约扩展 + D18 `orchestrationMode` 冻结 + D17 会话 DTO 冻结 | `packages/protocol` 公开 `TaskDto` / `dependsOn`；M7 扩展 draft/published `WorkflowDto`（nodes/edges 与现有 steps 同一协议）与 `TeamDto` / `TeamVersionDto` 写 payload。D18 启动字段 `orchestrationMode`（`workflow_bound \| direct`）已冻结在 `StartRunRequest`；省略默认 `workflow_bound`，未知值拒绝。D17 会话 / 草稿 DTO（`AuthoringSessionDto` / `AuthoringDraftDto`）已冻结；V0.1 传输为 Desktop-local / in-process；desktop-client **只导出类型**，无 chat HTTP path。DTO / 传输冻结 ≠ 编排 Agent / M7。本机会话存储 + 用户 append 已接线，仍无 Daemon chat path。字段冻结 ≠ M8；T21 项目启动面已探针（#26 `f3b2045` headed smoke PASS）+ composed start 透传（#28 仅 unit/composed，未跑 headed） |
...
| T20 | UI shell + 写 API 落草稿 + Desktop-local 会话存储 / 仅用户 append（编排 Agent 仍 planned） | `features/workflow-authoring/` 作者面、`InProcessAuthoringSessionStore` + 本机 `localStorage` 快照与 happy-dom 测试。工作流目录有「对话生成」入口：对照 `AuthoringSessionDto`；V0.1 传输为 Desktop-local / in-process，无 Daemon chat。用户可按 `AppendAuthoringSessionMessageInput` 追加消息（`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该本机切片）。#31 head `31d669be` 真窗 **FAIL**（`WORKFORCE-PR31-31d669be-AUTHORING-SESSION-STORE.md`）：SPA 导航能保住消息，renderer reload 丢会话。tip `c66eebe8` 复测 **PASS**（`WORKFORCE-PR31-c66eebe8-AUTHORING-SESSION-RETEST.md`：Ctrl+R 后同一会话 id + 用户消息仍在；无假 Agent 回复；草稿深链 OK）。`CHAT_SESSION_PROTOCOL_FROZEN=true` 仍只表示本机用户 append + 本地存储。结构化名称可走 `POST /workflows` + version 写接口落**未发布**草稿，失败保留输入与会话上下文。无假 Agent 回复、不发布、不执行。落草稿后深链 T18 画布路由 `/workflows/:id/versions/:versionId`。先前作者面壳 headed **PASS**（`66a9c284`）不覆盖本切片。**未实现**编排 Agent、对话生成、完整 headed 套件 |
| T21 | Desktop UI 探针 + composed start 透传（M8 未完成） | 双执行模式（D18）。`orchestrationMode` **已冻结**在 protocol `StartRunRequest`。**仅项目「开始执行」**有选择面并写入现有 `POST /projects/{id}:start`。composed `startProject` 把该字段传入 `app.start`，写入 `project.start` receipt / `ProjectRecord`，并在 `dispatchReadyTasks` / retry 时传入 `startRun` → `StartRunHostRequest` → `StartRunRequest`；`GET /projects/{id}` 与 `GET /runs` 可回读。`GET /capabilities` 声明 `orchestration.direct=false`，direct **disabled** / `422 unsupported_capability`。调度仍实例化已发布图，**没有** direct scheduler。**Task 详情不挂未接线控件**。**无**新 Run path。#26 tip `f3b2045` 项目「开始执行」UI / start path headed smoke **PASS**（`WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`：direct 诚实 disabled；`workflow_bound` 能启动 Run）。#28 composed 透传仅 unit / composed 测试，本切片**未跑** headed。**不是**完整 headed 套件。**不**宣称 M8 调度完成或生产 Codex direct |
...
# Test-bot 本机会话 reload headed PASS：WORKFORCE-PR31-c66eebe8-AUTHORING-SESSION-RETEST.md（验证 head c66eebe8）
# 先前 FAIL：WORKFORCE-PR31-31d669be-AUTHORING-SESSION-STORE.md（验证 head 31d669be）
# 不宣称真窗口对话编排 / Agent / M7 可用；编排 Agent 仍 planned
```
...
4. **桌面**  
   ...
   **T21 项目启动面 headed smoke：** `#26` / `f3b2045` **PASS**（`WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`：项目「开始执行」选 `orchestrationMode`；direct 诚实 disabled；`workflow_bound` 能启动 Run）。只覆盖该 UI / start 探针，不是完整 headed 套件，也不是 #28 透传的 headed 证据（透传仅 unit / composed）。
...
壳导航按更正后的 [IA §2](../product-ui/01-information-architecture.md)：**P0/P1 是切片深度，一级导航全部 `primary`**。IA 工作流用户目的现为「查看、编辑和发布可复用工作流（含对话生成与画布）」；**当前代码**是只读已发布目录 + T18 画布作者面 + T19 自定义 Team 写面 + T20 作者面（会话 DTO 与 Desktop-local 传输已冻结；本机会话存储 + 仅用户 append 已接线，编排 Agent 未接线）。Desktop IPC 已放行 GET 与矩阵写 path。未发布草稿不会出现在目录列表。T18 画布 headed 真窗 create/edit/save/publish 已 **PASS**（`fbe13dea`）。自定义 Team **桌面写面 UI + 草稿写 API 已接线**；headed 草稿 save→reload **PASS**（`406ee6d2`）；默认 `GET /teams` 只回 published。publish/自定义绑定按规则诚实显隐，无假成功。对话编排仍未实现。T21 只有项目启动面探针（#26 `f3b2045` headed smoke PASS）+ composed start 透传（#28 仅 unit/composed），不是 M8 完成。M7 **未完成**。不得宣称 Mock/Codex Runtime 可执行未发布图，也不得宣称 M7 / M8 完成或对话编排已接通。

## 5. 剩余工作

1. **Headed Electron 真窗口点击验收**：happy-dom / opt-in `executeJavaScript` helper **不能**代替人工。用 `pnpm --filter @workforce/desktop dev` 点目录对话框、SSE、Run 控制台、项目详情六标签与视觉。T18 画布、T19 草稿持久化、T20 本机会话 reload（`c66eebe8` / `WORKFORCE-PR31-c66eebe8-AUTHORING-SESSION-RETEST.md`）与 T21 项目「开始执行」UI smoke（`f3b2045` / `WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`）已有各自 headed PASS，不代替本条其余路径，也不是完整 headed 套件，也不是 #28 透传 headed，也不是编排 Agent。  
...
6. **M7 对话生成工作流（T20）未完成**：Desktop 作者面壳与写 API 落草稿已接通（#24 `edce1a4`）；headed 作者面壳 **PASS**（`66a9c284`）。会话 / 草稿 DTO **已冻结**；V0.1 传输 **已冻结**为 Desktop-local / in-process（无 Daemon chat path）。**本机会话存储 + 仅用户 append 已接线**（`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该切片）；快照写本机 `localStorage`。#31 `31d669be` 真窗 **FAIL**（Ctrl+R 丢会话）；tip `c66eebe8` 复测 **PASS**（`WORKFORCE-PR31-c66eebe8-AUTHORING-SESSION-RETEST.md`）。**不是**完整 headed 套件。**编排 Agent / 对话生成仍 planned**。不得把用户消息或结构化落草稿写成「对话编排已完成」，也不得渲染假 Agent 成功回复。
7. **M8 双执行模式未完成**：T21 **项目启动面 UI 已探针**，且 requested `orchestrationMode` **已写入**现有 start/Run/host 记录（不再在 HTTP 边静默丢掉）。**Task 详情不挂未接线控件**。`GET /capabilities` 现声明 `orchestration.direct=false`，因此 direct 不可点成功（`422 unsupported_capability`）。调度仍只走已发布图（缺省 `workflow_bound`）；**没有** direct 执行调度器。#26 tip `f3b2045` 项目「开始执行」headed smoke **PASS**（`WORKFORCE-TIP-f3b2045-ORCHESTRATION-START-SMOKE.md`）。#28 透传仅 unit / composed，**未跑** headed。**不是**完整 headed 套件。不得宣称 M8 完成或生产 Codex 已验证 direct。
