...
| T20 | UI shell + 写 API 落草稿 + Desktop-local 会话存储 / 仅用户 append（编排 Agent 仍 planned） | `features/workflow-authoring/` 作者面、进程内 `InProcessAuthoringSessionStore` 与 happy-dom 测试。工作流目录有「对话生成」入口：对照 `AuthoringSessionDto`；V0.1 传输为 Desktop-local / in-process，无 Daemon chat。用户可按 `AppendAuthoringSessionMessageInput` 追加消息（`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该本机切片）。结构化名称可走 `POST /workflows` + version 写接口落**未发布**草稿，失败保留输入与会话上下文。无假 Agent 回复、不发布、不执行。落草稿后深链 T18 画布路由 `/workflows/:id/versions/:versionId`。先前作者面壳 headed **PASS**（`66a9c284`）不覆盖本切片。**未实现**编排 Agent、对话生成、完整 headed 套件 |
...
# 不宣称真窗口对话编排可用；编排 Agent 仍 planned
```
...
6. **M7 对话生成工作流（T20）未完成**：Desktop 作者面壳与写 API 落草稿已接通（#24 `edce1a4`）；headed 作者面壳 **PASS**（`66a9c284`）。会话 / 草稿 DTO **已冻结**；V0.1 传输 **已冻结**为 Desktop-local / in-process（无 Daemon chat path）。**本机会话存储 + 仅用户 append 已接线**（`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该切片）。**编排 Agent / 对话生成仍 planned**。不得把用户消息或结构化落草稿写成「对话编排已完成」，也不得渲染假 Agent 成功回复。
