...
修订：2026-09-12 — T21 Desktop **双执行模式 UI 切片（部分接线）**：仅项目「开始执行」可选 `orchestrationMode` 并写入现有 `POST /projects/{id}:start`。`GET /capabilities` 未声明 `orchestration.direct` 时禁用 direct，无能力则 `422 unsupported_capability`。**Task 详情不挂未接线控件**。**不**宣称 M8 完成、headed PASS 或生产 Codex direct。#25 已 squash 进 main（`b700010`）：T02 冻结 D18 字段。T20 Desktop **UI shell + 写 API 落草稿**已在 main。作者面壳 headed **PASS**（`66a9c284`）。T18 / T19 headed PASS 保留。M7 **未完成**。D19 远程 enrollment 与容器 runner **未实现**。
