...
4. **桌面**  
   ...
   **T21 项目启动面 headed smoke：** Test-bot 在 `#26` / `f3b2045` **PASS**（项目详情选 `orchestrationMode` 后 `:start`）。只覆盖这条项目启动探针，不是完整 headed 套件，也不是 M8 调度或生产 Codex direct。
