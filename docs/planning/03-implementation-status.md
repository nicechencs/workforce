...
| ID | 状态 | 证据 |
|---|---|---|
| T00 | 完成（项目制 + M7/M8 + D19 Placement 决策已补写） | M0–M3 冻结仍有效；§0 项目制；D15/D16 画布与自定义 Team；D17 对话生成（M7 planned，仅有 UI shell；会话 DTO 与 Desktop-local 传输已冻结，发送未接线）；D18 双执行模式（M8 planned；`orchestrationMode` 字段已冻结；T21 项目启动面已探针（#26 `f3b2045` headed smoke PASS）+ composed start 透传（#28 仅 unit/composed），M8 未完成）；D19 本机默认、远程与容器为一等 Placement。M7 **写 API + 协议**已接通。T18 画布 UI **已接线**，headed 真窗 create/edit/save/publish **PASS**（`fbe13dea`）。T19 桌面写面 + 草稿持久化 **已接线**，headed save→reload **PASS**（`406ee6d2`）。**未实现**对话发送 / 编排 Agent（T20 剩余）、M8 调度 / 生产 Codex direct、远程 enrollment 与容器 runner。M7 未完成 |
| T01 | 完成 | pnpm + turbo monorepo；本轮补了 Electron/React/Vite lockfile |
| T02 | 完成（M3 字段）+ M7 写契约扩展 + D18 `orchestrationMode` 冻结 + D17 会话 DTO 冻结 | `packages/protocol` 公开 `TaskDto` / `dependsOn`；M7 扩展 draft/published `WorkflowDto`（nodes/edges 与现有 steps 同一协议）与 `TeamDto` / `TeamVersionDto` 写 payload。D18 启动字段 `orchestrationMode`（`workflow_bound \| direct`）已冻结在 `StartRunRequest`；省略默认 `workflow_bound`，未知值拒绝。D17 会话 / 草稿 DTO（`AuthoringSessionDto` / `AuthoringDraftDto`）已冻结；V0.1 传输为 Desktop-local / in-process；desktop-client **只导出类型**，无 chat HTTP path。DTO / 传输冻结 ≠ T20 发送 / 编排 Agent / M7。字段冻结 ≠ M8；T21 项目启动面已探针（#26 `f3b2045` headed smoke PASS）+ composed start 透传（#28 仅 unit/composed，未跑 headed） |
...
| T20 | UI shell + 写 API 落草稿（会话 DTO 与 Desktop-local 传输已冻结；发送仍禁用） | `features/workflow-authoring/` 作者面与 happy-dom 测试。工作流目录有「对话生成」入口：可编译对照 `AuthoringSessionDto`；V0.1 传输为 Desktop-local / in-process，无 Daemon chat。发送仍禁用（`CHAT_SESSION_PROTOCOL_FROZEN=false`）。结构化名称可走 `POST /workflows` + version 写接口落**未发布**草稿，失败保留输入。无假 Agent 回复、不发布、不执行。落草稿后深链 T18 画布路由 `/workflows/:id/versions/:versionId`。Test-bot 作者面壳 headed **PASS**（`66a9c284`，`WORKFORCE-PR24-66a9c284-AUTHORING-SHELL-RETEST.md`；先前 `7f85c503` / `WORKFORCE-PR24-7f85c503-AUTHORING-SHELL.md`）。**未实现**发送接线、编排 Agent、对话生成、完整 headed 套件 |
...
壳导航按更正后的 [IA §2](../product-ui/01-information-architecture.md)：**P0/P1 是切片深度，一级导航全部 `primary`**。IA 工作流用户目的现为「查看、编辑和发布可复用工作流（含对话生成与画布）」；**当前代码**是只读已发布目录 + T18 画布作者面 + T19 自定义 Team 写面 + T20 作者面壳（会话 DTO 与 Desktop-local 传输已冻结，发送未接线）。Desktop IPC 已放行 GET 与矩阵写 path。未发布草稿不会出现在目录列表。T18 画布 headed 真窗 create/edit/save/publish 已 **PASS**（`fbe13dea`）。自定义 Team **桌面写面 UI + 草稿写 API 已接线**；headed 草稿 save→reload **PASS**（`406ee6d2`）；默认 `GET /teams` 只回 published。publish/自定义绑定按规则诚实显隐，无假成功。对话编排仍未实现。T21 只有项目启动面探针（#26 `f3b2045` headed smoke PASS）+ composed start 透传（#28 仅 unit/composed），不是 M8 完成。M7 **未完成**。不得宣称 Mock/Codex Runtime 可执行未发布图，也不得宣称 M7 / M8 完成或对话编排已接通。
...
6. **M7 对话生成工作流（T20）未完成**：Desktop 作者面壳与写 API 落草稿已接通（#24 `edce1a4`）；headed 作者面壳 **PASS**（`66a9c284`，`WORKFORCE-PR24-66a9c284-AUTHORING-SHELL-RETEST.md`）。会话 / 草稿 DTO **已冻结**；V0.1 传输 **已冻结**为 Desktop-local / in-process（无 Daemon chat path）。**发送 / 编排 Agent / 对话生成仍 planned**。不得把结构化落草稿或只读目录写成「对话编排已完成」，也不得渲染假 Agent 成功回复。后续须 Desktop 会话存储 + 仅用户 append 接线后才能打开发送；DTO / 传输冻结不是对话路径接通，也不得把 `CHAT_SESSION_PROTOCOL_FROZEN` 翻成 true。
