...
| 对话生成工作流草稿 | M7 planned（D17）。用户对话生成 bot/角色/流程/任务草稿，再进画布编辑。会话 / 草稿 DTO 已冻结；V0.1 传输为 Desktop-local / in-process；**无** chat HTTP path / Daemon chat 资源；作者面壳发送仍禁用，不得假成功 |
...
对话生成（D17）**不在本表发明 path**。会话 / 草稿 DTO 已冻结为 `AuthoringSessionDto` / `AuthoringDraftDto`（`packages/protocol`）；V0.1 传输已冻结为 **Desktop-local / in-process**。矩阵只命名该 DTO，**不增加** chat 资源，也**不**登记 Daemon chat 路由。后续 T20 发送接线对照该 DTO 做 Desktop 会话存储 + 仅用户 append，不是公开 HTTP chat。D18 启动字段已冻结为 `orchestrationMode`（`packages/protocol` `StartRunRequest`）；矩阵仍只保留 planned 页面行，**不增加** Run path。实现时优先复用已列的 M7 workflow 写接口与现有 `GET /capabilities` / Run 启动命令，而不是另开未登记的 chat 或 `:direct` 资源。T20 发送仍禁用，DTO / 传输冻结 ≠ 对话生成完成。
...
| 对话生成工作流草稿 | 复用上列 M7 写接口落草稿；会话 DTO 已冻结；V0.1 Desktop-local / in-process；无 chat path | M7 planned（D17）。Desktop **作者面壳 + 写 API 落草稿**已有；发送仍禁用（`CHAT_SESSION_PROTOCOL_FROZEN=false`），不得假 Agent 成功。后续发送 ≠ Daemon chat 资源 |
...
- T20 作者面壳不得渲染假 Agent 成功；会话 DTO 与 Desktop-local 传输已冻结仍不发明 chat path / Daemon chat 资源；发送未接线前保持禁用
