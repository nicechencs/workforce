...
修订：2026-09-12 — T20 Desktop-local 会话存储 + 仅用户 append 已接线。本矩阵**仍不增加** chat HTTP path，也**不**登记 Daemon chat 资源。`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该本机切片；编排 Agent 回复仍 planned。同日更早：D17 V0.1 会话传输冻结为 **Desktop-local / in-process**（对照已冻结 `AuthoringSessionDto` / `AuthoringDraftDto`）。同日 T21 / D18 composed `:start` 透传：`orchestrationMode` 写入现有 `project.start` receipt / `ProjectDto` / `RunDto` / host `StartRunRequest`。`GET /capabilities` 的 `orchestration.direct` 仍为 false。**仍不增加** `/runs/{id}:direct`、chat 或其它新 Run path。M8 **未完成**（无 direct 调度、无完整 headed 套件、无生产 Codex direct）。同日更早：T21 Desktop UI 切片（#26 `f3b2045`，项目启动面 headed smoke PASS）；现有 `POST /projects/{id}:start` 接受该字段。T20 作者面壳 + 写 API 落草稿已在 main（#24 `edce1a4`）。D18 字段已由 T02 冻结为 `orchestrationMode`（`workflow_bound | direct`）。2026-09-11 — 补 D19：Placement kind = `local`（默认）/ `remote` / `container`。本矩阵**不增加** enrollment、远程 lease 或 Docker/K8s path；`GET /nodes` 仍仅 Local Node。同日主对象是 Project。画布与自定义 Team 从 `later` 迁出，列入 **M7**，作为项目循环（Team → Tasks → Workflow）的必达环节，不是外挂页。只读 `GET /workflows` 是已接通的 M3/P1 过渡目录（不是 Mock 闭环硬依赖，也不是可执行 Runtime）。同日补公开 Task DTO 的 `dependsOn` 与 Artifact 权威存储规则（`LocalArtifactStore`）；未发明可写项目策略或远程 enrollment。同日冻结 D17 对话生成（**M7 planned**）与 D18 双执行模式（**M8 planned**）。

**项目制：** 页面与 API 围着 Project。M3 用预设 Team + 只读工作流目录走完 Mock 项目闭环。M7 补齐「给该项目编排 Team / 用画布或对话生成并编辑 Workflow」。M8 补齐「Agent 跟随已发布工作流或直接执行」。D17 会话 DTO **已冻结**；V0.1 传输 **Desktop-local / in-process**（无 Daemon chat 资源）；本机会话存储 + 仅用户 append **已接线**；编排 Agent **仍未实现**。T21 仅有项目启动面探针 + start 透传，M8 **未完成**。
...
- **M8**：按 Agent 双执行模式（`orchestrationMode`: `workflow_bound | direct`；产品必达；见 D18）。字段已冻结并写入现有 start/Run 记录；T21 **UI 切片已接线**（无能力则禁用）。项目启动面 headed smoke **PASS**（`f3b2045`）。调度 / 完整 headed 套件 / 生产 Codex direct **未完成**，不发明 path
...
| 对话生成工作流草稿 | M7 planned（D17）。用户对话生成 bot/角色/流程/任务草稿，再进画布编辑。会话 / 草稿 DTO 已冻结；V0.1 传输为 Desktop-local / in-process；**无** chat HTTP path / Daemon chat 资源；本机会话可追加用户消息，编排 Agent 仍 planned，不得假成功 |
...
| 页面/动作 | 处理 |
|---|---|
| 按 Agent 选择执行模式 | M8 planned（D18）。字段已冻结：`orchestrationMode` = `workflow_bound`（跟随已发布 WorkflowVersion）或 `direct`（绕过该次图、即席执行），位于现有 `StartRunRequest` / `POST /projects/{id}:start`，并回读于 `GET /projects/{id}` 与 `GET /runs`。靠 `GET /capabilities`（`orchestration.direct`）；无能力则禁用或 `422 unsupported_capability`。**不发明** `/runs/{id}:direct`。T21 UI + start 透传 **已接线**；项目启动面 headed smoke **PASS**（`f3b2045`）。调度 / 完整 headed 套件 / 生产 Codex direct **未完成** |
...
对话生成（D17）**不在本表发明 path**。会话 / 草稿 DTO 已冻结为 `AuthoringSessionDto` / `AuthoringDraftDto`（`packages/protocol`）；V0.1 传输已冻结为 **Desktop-local / in-process**。矩阵只命名该 DTO，**不增加** chat 资源，也**不**登记 Daemon chat 路由。T20 已对照该 DTO 做 Desktop 会话存储 + 仅用户 append，不是公开 HTTP chat。D18 启动字段已冻结为 `orchestrationMode`（`packages/protocol` `StartRunRequest`）；矩阵仍只保留 planned 页面行，**不增加** Run path。实现时优先复用已列的 M7 workflow 写接口与现有 `GET /capabilities` / Run 启动命令，而不是另开未登记的 chat 或 `:direct` 资源。编排 Agent 仍未接线，DTO / 传输冻结 ≠ 对话生成完成。
...
| 对话生成工作流草稿 | 复用上列 M7 写接口落草稿；会话 DTO 已冻结；V0.1 Desktop-local / in-process；无 chat path | M7 planned（D17）。Desktop **作者面壳 + 写 API 落草稿 + 本机会话 / 仅用户 append**已有（`CHAT_SESSION_PROTOCOL_FROZEN=true` 只表示该切片），不得假 Agent 成功。用户 append ≠ Daemon chat 资源 |
| 发布工作流版本 | `/workflows/{id}/versions/{versionId}:publish` | M7；有限 DAG 校验通过 |
| 选择绑定工作流或直接执行 | 现有 `POST /projects/{id}:start` 上的 `orchestrationMode` + `GET /capabilities` | M8 planned（D18）。字段已冻结并写入现有 start/Run 记录；T21 UI 切片已接线。无能力则禁用。无新 path。不宣称 M8 完成 |
...
- T20 作者面不得渲染假 Agent 成功；会话 DTO 与 Desktop-local 传输已冻结仍不发明 chat path / Daemon chat 资源；本机会话只追加用户消息，编排 Agent 未接线
- T21 项目启动面探针 + start/Run 透传已接线不等于 M8 完成；Task 详情不挂未接线控件；无 `orchestration.direct` 不得假 mode；不发明 `:direct` path
