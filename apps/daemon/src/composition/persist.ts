...
export async function loadComposition(
  ...
  const world: PersistedWorld = {
    ...
    runs: entities.runs.map((run) => {
      const stored =
        sqliteHandlesByRunId.get(run.id) ??
        sqliteHandles.find((handle) => handle.request.operationId === run.operationId);
      const handleId = stored?.handle.handleId ?? handlesByOperation.get(run.operationId);
      const orchestrationMode =
        stored?.request.orchestrationMode ??
        base.runs.find((item) => item.id === run.id)?.orchestrationMode;
      return {
        ...run,
        ...(handleId ? { handleId } : {}),
