...
describe("composed orchestrationMode pass-through", () => {
  it("threads workflow_bound into project.start / Run / host request and still refuses unsupported direct", async () => {
    const harness = await startComposed();
    const { daemon, auth, services } = harness;
    const port = daemon.port;
    ...
    const refused = await json(port, `/api/v1/projects/${project.id}:start`, {
      method: "POST",
    ...
    const run = await poll(async () => {
      ...
      return (
        listed.body as {
          items: Array<{ id: string; orchestrationMode?: string }>;
        }
      ).items.find((item) => item.orchestrationMode === "workflow_bound");
    });
    expect(run.orchestrationMode).toBe("workflow_bound");

    const receipt = await json(port, "/api/v1/operations/op_orch_comp_bound", { headers: auth });
