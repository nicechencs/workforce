...
describe("workflow-authoring protocol honesty", () => {
  it("wires user-append + local store without inventing a Daemon chat path", () => {
    ...
    expect(CHAT_SESSION_GAP).toContain("CHAT_SESSION_PROTOCOL_FROZEN");
...
describe("authoring UI", () => {
  ...
  it("keeps the workflows entry honest: Agent generate disabled, local store ready, flag false", () => {
    const html = renderToStaticMarkup(
