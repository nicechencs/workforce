...
/**
 * Desktop-local session store + user-only append are wired. This flag stays
 * false: Agent / send is not wired, and no Daemon chat HTTP path exists.
 */
export const CHAT_SESSION_PROTOCOL_FROZEN = false;

export const CHAT_SESSION_GAP =
  "Desktop-local / in-process 会话存储已接线：用户消息按 AppendAuthoringSessionMessageInput 追加，对照已冻结 AuthoringSessionDto / AuthoringDraftDto；快照写入本机 localStorage，跨 renderer reload 仍可恢复。CHAT_SESSION_PROTOCOL_FROZEN 仍为 false。能力矩阵仍不列 chat endpoint / Daemon chat 资源。「发送给编排 Agent」仍禁用。编排 Agent 回复仍 planned，禁止把假回复渲染成生成成功。";
